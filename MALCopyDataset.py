...
class MALDataset(Dataset):
    def __init__(self, files, src_vocab2id, tokenizer, n=1E10):
        ...
        self.tgt_vocab2id=src_vocab2id
        self.files=files
        self.n=n
    ...
    def load(self):
        with codecs.open(self.files[0], encoding='utf-8') as f:
            ...
            for id in range(len(data)):
                ...
                bg_dyn_vocab2id, bg_dyn_id2vocab = build_vocab(self.tokenzier(sample['background'].lower().replace("<eor>", '')))
                self.bg_dyn_vocab2ids.append((id, bg_dyn_vocab2id))
                self.bg_dyn_id2vocabs.append((id, bg_dyn_id2vocab))

                output = sample['response'].lower().replace('<eor>', '')
                output = self.tokenzier(output)
                self.outputs.append(output)
                self.output_arrays.append(torch.tensor([self.tgt_vocab2id.get(w.lower(), UNK) for w in output] + [SEP], requires_grad=False).long())
                self.background_copy_arrays.append(torch.tensor([bg_dyn_vocab2id.get(w.lower(), UNK) for w in output] + [SEP], requires_grad=False).long())
...
def train_collate_fn(data):
    ...
    def one_instance(b):
        ...
        for j in range(len(background[b])):
            background_map_t[b, j, vocab2id[background[b][j]]] = 1

    for b in range(batch_size):
        one_instance(b)

    return {'id':id_t,'context':context_t, 'query':query_t, 'output':output_t,'output_text':output_text, 'background':background_t,
            'background_text': background_text, 'background_ref_start':background_ref_start_t,'background_ref_end':background_ref_end_t,
            'background_dyn_vocab':dict(bg_dyn_id2vocab), 'background_copy':background_copy_t, 'background_map':background_map_t}
