...
class MALDataset(Dataset):
    ...
    def __getitem__(self, index):
        return [self.ids[index], self.id_arrays[index], self.contexts[index], self.context_arrays[index], self.queries[index], self.query_arrays[index],
                self.backgrounds[index], self.background_arrays[index], self.outputs[index], self.output_arrays[index], self.background_ref_start_arrays[index], self.background_ref_end_arrays[index]]
...
def train_collate_fn(data):
    id,id_a,context,context_a,query,query_a,background,background_a,output,output_a, background_ref_start_a, background_ref_end_a = zip(*data)
    ...
    return {'id':id_t, 'context':context_t, 'query':query_t, 'output':output_t, 'background':background_t, 'background_ref_start':background_ref_start_t,'background_ref_end':background_ref_end_t}
...
def test_collate_fn(data):
    ...
    def one_instance(b):
        ...
        background_text[id[b]]=background[b]
    ...
    return {'id': id_t, 'context': context_t, 'query':query_t, 'output': output_t, 'background': background_t, 'background_text': background_text}
