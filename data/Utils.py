...
def build_vocab(words):
    dyn_vocab2id = dict(vocab2id)
    dyn_id2vocab = dict(id2vocab)
    for w in words:
...
def tokens2chars(tokens, char2id, word_len=15):
    tmp = torch.zeros(len(tokens), word_len).long()
    for i, word in enumerate(tokens):
        for j, ch in enumerate(word):
            if j>=word_len:
                break
            tmp[i][j] = char2id[ch] if ch in char2id else UNK
    return tmp

char2id=dict(vocab2id)
id2char=dict(id2vocab)
def load_char():
...
def load_vocab(vocab_file,t=0):
    thisvocab2id = dict(vocab2id)
    thisid2vocab = dict(id2vocab)
    id2freq = {}
    ...
    with codecs.open(vocab_file, encoding='utf-8') as f:
        for line in f:
            try:
                ...
                continue
            if int(freq)>=t:
                id=len(thisvocab2id) # the id for the next word
                thisvocab2id[name] = id # word: id
                thisid2vocab[id] = name # id: word
                id2freq[id]=int(freq) # id: freq
                sum_freq+=int(freq) # sum of freq
    id2freq[0] = sum_freq/len(id2freq)  # sum_freq / sum_words
    id2freq[1] = id2freq[0]
    id2freq[2] = id2freq[0]
    id2freq[3] = id2freq[0]
