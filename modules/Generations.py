...
def sample(model, data, max_len=20):
    ...
    init_decoder_input = new_tensor([CLS] * batch_size, requires_grad=False)
...
def greedy(model,data,max_len=20):
    ...
    decoder_input = new_tensor([CLS] * batch_size, requires_grad=False)
    ...
    for t in range(max_len):
        ...
        greedy_indice = ids[:,0]
        greedy_this_end = greedy_indice == SEP
        if t == 0:
...
def beam(model,data,max_len=20,width=5):
    ...
    for i in range(batch_size):
        next_fringe += [Node(parent=None, state=[s[i].unsqueeze(0) for s in decoder_states], word=CLS_WORD, value=CLS, cost=0.0, encode_outputs=[o[i].unsqueeze(0) for o in encode_outputs], data=get_data(i,data), batch_id=i)]
        results[i] = []

    for l in range(max_len+1):
        fringe = []
        for n in next_fringe:
            if n.value == SEP or l == max_len:
...
def transformer_greedy(model,data,max_len=20):
    ...
    decoder_input = new_tensor([CLS] * batch_size).view(batch_size, 1)
...
def transformer_beam(model,data,max_len=20,width=5):
    ...
    results = dict()
    for i in range(batch_size):
        next_fringe += [Node(parent=None, state=None, word=CLS_WORD, value=CLS, cost=0.0, encode_outputs=[o[i].unsqueeze(0) for o in encode_outputs], data=get_data(i,data), batch_id=i)]
        results[i] = []
