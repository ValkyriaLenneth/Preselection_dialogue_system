...
class BilinearAttention(nn.Module):
    def __init__(self, query_size, key_size, hidden_size, dropout=0.5, coverage=False, gumbel=False, temperature=100):
        super().__init__()
        self.linear_key = nn.Linear(key_size, hidden_size, bias=False) # H * H
        self.linear_query = nn.Linear(query_size, hidden_size, bias=True) # H * H
        self.v = nn.Linear(hidden_size, 1, bias=False) # H * 1
    ...
    def score(self, query, key, query_mask=None, key_mask=None, mask=None, sum_attention=None):
        batch_size, key_len, key_size = key.size() # B * 1 * H
        batch_size, query_len, query_size = query.size() # B * lc * H
    ...
    def unnormalized_score(self, query, key, key_mask=None, mask=None, sum_attention=None):
        ...
        wq = self.linear_query(query.view(-1, query_size)) # B * H
        wq = wq.view(batch_size, query_len, 1, self.hidden_size) # B * 1 * 1 * H
        wq = wq.expand(batch_size, query_len, key_len, self.hidden_size) # B * 1 * lc * H

        uh = self.linear_key(key.view(-1, key_size)) # (B*lc) * H
        uh = uh.view(batch_size, 1, key_len, self.hidden_size) # B * 1 * lc * H
        uh = uh.expand(batch_size, query_len, key_len, self.hidden_size) # B * 1 * lc * H
